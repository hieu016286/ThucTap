        ...
        <div class="card-body">
            <form action="{{ route('roles.update',['id'=>$role->id]) }}" method="post">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control rounded-0" name="name" placeholder="Role Name..." value="{{ $role->name }}">
                    @if($errors->has('name'))
                        <p class="text-danger">{{$errors->first('name')}}</p>
                    @endif
                    <label for="name">Description:</label>
                    <textarea  class="form-control rounded-0" name="display_name" placeholder="Description..." >{{$role->display_name}}</textarea>
